...
## Библиотечный файловый ввод-вывод

* Бинарный поток - последовательность байт, текстовый - последовательность символов, разделённых '\n'
* Поток может быть ассоциирован с файлом, консолью, устройством
* С каждым открытым потоком сопоставляется FILE
* У каждого потока - текущая позиция, индикатор конца файла и индикатор ошибки
* Уровни буферизации
* fputc - функция (расходы на вызов), putc - может быть макросом
* fseek/ftell, fgetpos/fsetpos
* Цикл по feof
* Особенности работы в режимах "a", "a+"
* Сдвиг за конец файла, файл с дырками
* Библиотечные функции чтения-записи используют внутренние буферы, поэтому даже чтение по одному символу эффективно (за каждым символом не происходит специального обращения к диску, символ считывается из внутреннего буфера) 
...
Задачи:
* realloc кусочками - копируем информацию порциями байт из строки в строку
* скопировать argv
* создать матрицу как одномерный массив, заполнить главную и побочные диагонали

## Работа со строками
Массив, заполняемый строкой; модифицировать можно:
~~~c
char s[] = "Hello world";
~~~

Указатель на строковую константу; модифицировать нельзя:
~~~c
char *s = "Hello world";
~~~
